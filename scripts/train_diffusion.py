...
import sys
sys.path.append('/Users/peterobi/Documents/targetdiff/targetdiff')
...
if __name__ == '__main__':
    ...
    shutil.copytree('./models', os.path.join(log_dir, 'models'))
      ...
      # Transforms
    rna_featurizer = trans.FeaturizeRNAAtom()  # Make sure to define this function for RNA
    ...
    # follow_batch is changed to reflect RNA names
    follow_batch = ['rna_element', 'ligand_element']
    ...
    model = ScorePosNet3D(
        config.model,
        protein_atom_feature_dim=rna_featurizer.feature_dim,
        ligand_atom_feature_dim=ligand_featurizer.feature_dim
    ).to(args.device)
    # print(model)
    print(f'protein feature dim: {rna_featurizer.feature_dim} ligand feature dim: {ligand_featurizer.feature_dim}')
    ...
    def train(it):
        ...
        for _ in range(config.train.n_acc_batch):
            ...
            protein_noise = torch.randn_like(batch.rna_pos) * config.train.pos_noise_std
            gt_protein_pos = batch.rna_pos + protein_noise
            results = model.get_diffusion_loss(
                protein_pos=gt_protein_pos,
                protein_v=batch.rna_atom_feature.float(),
                batch_protein=batch.rna_element_batch,
